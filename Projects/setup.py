...
flywheel_script = os.path.join(base_dir, "flywheel.py")
bat_path = os.path.join(base_dir, "flywheel.bat")

# Create the launcher batch file
try:
    with open(bat_path, "w") as f:
        f.write(f'@echo off\npython "{flywheel_script}" %*\n')
    print(f"[+] Created launcher: {bat_path}")
except FileExistsError:
    print("[*] Did not create flywheel.bat, file already exists.")
